...
public class MulticastReceiver extends Thread {
            ...
            threadloop: while (exit == false) {
                ...
                while (this.holdbackQueue.hasMoreDeliverables(senderUuid)) {
                    ...
                    if (object instanceof Message) {
                        ...
                        if (object instanceof SubscriptionMessage) {
                            ...
                            if (topic.getLeader().getIPAdress().equals(localHostAdress)) {
                                ...
                                RightNeighbor myFormerRightNeighbor = App.rightTopicNeighbours.replace(topic.getUUID(),
                                        new RightNeighbor(subscriberAdress));
                                ...
                                if (myFormerRightNeighbor != null) {
                                    multicastPublisher.sendTopicNeighborUnicast(subscriberAdressString,
                                            new TopicNeighbor(topic, myFormerRightNeighbor,
                                                    new LeftNeighbor(localHostAdress)));

                                    multicastPublisher.sendTopicNeighborUnicast(
                                            myFormerRightNeighbor.getIPAdress().getHostAddress(),
                                            new TopicNeighbor(topic, new RightNeighbor(null),
                                                    new LeftNeighbor(subscriberAdress)));
                                } else {
                            ...
                            if (topic.getLeader().getIPAdress().equals(localHostAdress)) {
                                ...
                                InetAddress unsubscriberAdress = InetAddress
                                        .getByName(unsubscriptionMessage.getContent());
                                ...
                                if (rightNeighborOfUnsubscriber.getIPAdress()
                                        .equals(leftNeighborOfUnsubscriber.getIPAdress())) {
                                    // When only two nodes are left and one leaves, the last node will receive null
                                    // as new neighbors.
                                    multicastPublisher.sendTopicNeighborUnicast(
                                            rightNeighborOfUnsubscriber.getIPAdress().getHostAddress(),
                                            new TopicNeighbor(topic, null, null));
                                } else {
                                    multicastPublisher.sendTopicNeighborUnicast(
                                            rightNeighborOfUnsubscriber.getIPAdress().getHostAddress(),
                                            new TopicNeighbor(topic, new RightNeighbor(null),
                                                    leftNeighborOfUnsubscriber));

                                    multicastPublisher.sendTopicNeighborUnicast(
                                            leftNeighborOfUnsubscriber.getIPAdress().getHostAddress(),
                                            new TopicNeighbor(topic, rightNeighborOfUnsubscriber,
                                                    new LeftNeighbor(null)));
                                }
                            }
                            ...
                            InetAddress heartbeatSenderAdress = InetAddress
                                    .getByName(hearbeatFailedMessage.getContent());
                            InetAddress failedNeighborAdress = hearbeatFailedMessage.getMissingRightNeighbor()
                                    .getIPAdress();
                            ...
                            if (currentLeftNeighbor.getIPAdress().equals(failedNeighborAdress)) {
                                App.leftTopicNeighbours.replace(topic.getUUID(), currentLeftNeighbor,
                                        new LeftNeighbor(heartbeatSenderAdress));

                                multicastPublisher.sendTopicNeighborUnicast(heartbeatSenderAdress.getHostAddress(),
                                        new TopicNeighbor(topic, new RightNeighbor(IPAdress.getLocalIPAddress()),
                                                new LeftNeighbor(null)));
                            }
                        } else {
                    ...
                    if (object instanceof TopicNeighbor) {
                        ...
                        if (rightNeighbor != null) {
                            if (rightNeighbor.getIPAdress() != null) {
                                App.rightTopicNeighbours.put(topicNeighbor.getTopic().getUUID(),
                                        topicNeighbor.getRightNeighbor());
                            }
                        } else {
                        ...
                        if (leftNeighbor != null) {
                            if (leftNeighbor.getIPAdress() != null) {
                                App.leftTopicNeighbours.put(topicNeighbor.getTopic().getUUID(),
                                        topicNeighbor.getLeftNeighbor());
                            }
                        } else {
                    ...
                    if (object instanceof RingCompleteMessage) {
                        ...
                        // Create new topicNode for leader election purposes. If node already exists,
                        // put will return existing one.
                        ...
                        multicastPublisher.sendMessageUnicast(
                                App.rightTopicNeighbours.get(topic.getUUID()).getIPAdress().getHostAddress(),
                                nodeMessage);
                    }
                }
            }
    ...
    private String deserialize(InetAddress sender, byte[] receivedData) {
        try {
            ...
            // Preserve UnicastRequests to be pushed to holdback queue
            if (object instanceof UnicastRequest) {
                UnicastRequest request = (UnicastRequest) object;
                request.setSender(sender);
                this.executeRequest(request.getPayload());
    ...
    private void executeRequest(Object object) throws IOException {
        // "End" topic
        if (object instanceof Topic && ((Topic) object).getName().equals("end")) {
            Log.debug("Received end topic. Ready to die now...");
        }
            ...
            if (object instanceof SubscriptionMessage) {
                ...
                if (topic.getLeader().getIPAdress().equals(localHostAdress)) {
                    ...
                    if (myFormerRightNeighbor != null) {
                        ...
                        multicastPublisher.sendTopicNeighborUnicast(subscriberAdressString, new TopicNeighbor(topic,
                                new RightNeighbor(localHostAdress), new LeftNeighbor(localHostAdress)));
                    }
                }
            } else if (object instanceof UnsubscriptionMessage) {
                UnsubscriptionMessage unsubscriptionMessage = (UnsubscriptionMessage) message;
        ...
        if (object instanceof RingCompleteMessage) {
            ...
            TopicNodeMessage nodeMessage = new TopicNodeMessage(topic, topicNode.getUUID(), false);
            multicastPublisher.sendMessageUnicast(
                    App.rightTopicNeighbours.get(topic.getUUID()).getIPAdress().getHostAddress(), nodeMessage);
        }
    }
}